...
class DataGeneratorGait(keras.utils.Sequence):
	...
	def __init__(self, dataset_info, batch_size=128, mode='train', balanced_classes=True, use3D=False, labmap=[], modality='of',
	             ...
	             lstm=False, nframes=None, keep_data=False, mirror=False, mean_sample=0, std_sample=1, gaitset=False):
		'Initialization'
		self.use3D = use3D
		...
		if True:  # modality != 'silhouette':  # mjmarin: why is this condition here?
	...
	def __getitem__(self, index):
		...
		if self.gaitset:
			new_X = np.zeros((X.shape[0], int(X.shape[1]/2), X.shape[2], X.shape[3], 2), dtype=X.dtype)
	...
	def __getitemvideoid__(self, index):
		...
		fnames = []
		for ix in self.batches[index]:
			if self.mirror:
				fnames.append(self.file_list[ix]+"-mirror")
			else:
				fnames.append(self.file_list[ix])
		...
		return X, y, videoId, cameras, fnames
	...
	def on_epoch_end(self):
		if self.balanced:
			...
			for i in range(self.__len__()):
				...
				# The remaining samples are selected randomly from the different kinds of gaits.
				if self.mode != 'test':
			...
			self.batches = []
			# nbs = int(np.floor( len(self.file_list) / len(self.batch_size )))
			for bix in range(self.__len__()):
				if bix < (self.__len__()-1):
					self.batches.append(np.arange(bix*self.batch_size, (bix+1)*self.batch_size))
				else:
					self.batches.append(np.arange(bix*self.batch_size, len(self.file_list)))
	...
	def __load_dd(self, filepath: str, clip_max=0, clip_min=0):
		...
		if sample["compressFactor"] > 1:
			...
			if self.modality == 'gray' or self.modality == 'depth':
